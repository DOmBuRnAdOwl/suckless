...
match(void)
{
	...
	int i, tokc = 0;
	size_t len, textsize;
	struct item *item, *lhpprefix, *lprefix, *lsubstr,*lhpsubstr, *hpprefixend, *prefixend, *substrend,*hpsubstrend;
	strcpy(buf, text);
	...
	matches = lhpprefix = lprefix = lhpsubstr = lsubstr = matchend = hpprefixend = prefixend = hpsubstrend = substrend = NULL;
	...
	for (item = items; item && item->text; item++) {
		...
		if (i != tokc) /* not all tokens match */
			continue;
		/* exact matches go first, then prefixes with high priority,then substrings with priority, then prefixes, then substrings */
		if (!tokc || !fstrncmp(text, item->text, textsize)) // exact match
			appenditem(item, &matches, &matchend);
		else if (item->hp && !fstrncmp(tokv[0], item->text, len)) //pref prior
			appenditem(item, &lhpprefix, &hpprefixend);
		else if (item->hp && !fstrstr(tokv[0], item->text)) //substr prior
			appenditem(item, &lhpsubstr, &hpsubstrend); // substr
		else if (!fstrncmp(tokv[0], item->text, len)) //pref
			appenditem(item, &lprefix, &prefixend);
		else
			appenditem(item, &lsubstr, &substrend); // substr
	}
	if (lhpprefix) {
		if (matches) {
	...
	if (lhpsubstr) {
		if (matches) {
			matchend->right = lhpsubstr;
			lhpsubstr->left = matchend;
		} else
			matches = lhpsubstr;
		matchend = hpsubstrend;
	}
	if (lprefix) {
		if (matches) {
